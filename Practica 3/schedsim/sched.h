...
typedef struct {
	...
	int prio;						/* Task's priority 
									...
									*/
	task_state_t state;
	int last_cpu;					/* CPU where the task ran last time // Última CPU donde se ha ejecutado una tarea*/
	...
	int runnable_ticks_left;		/* Number of ticks the application has to complete till blocking or exiting 
										// Se usa para saber cuál es la proxima tarea a ejecutar
									*/
	...
	bool on_rq;						/* Marker to check if the task is on the rq or not !!  
										// Boolean para saber si la tarea está o no en la run queue, es decir
										no está en ejecución ni bloqueada
									*/
	unsigned long flags;			/* generic flags field 
										// TF_IDLE_TASK -> Sirve para saber si una tarea es libre
										// TF_INSERT_FRONT -> Usa SJF para introducir una tarea al principio de la lista
									*/
	void* tcs_data;					/* Pointer enabling a scheduling class to store private data if needed 
										// Permite que el algoritmo apunte a una estructura con campos privados
										// Solo lo usa RR, los demás lo tiene a NULL
									*/
	/* General statistics */	
...
typedef struct {
	slist_t tasks;   			/* runnable task queue 
									// Lista de tareas para ejecutar la CPU 
								*/
	task_t* cur_task;			/* Puntero a la tarea actual que está ejecutando la CPU 
									// Esta tarea no está incluida en la lista de tareas task 
									// La tarea puede ser vacía/inactiva
								*/
	task_t idle_task;			/* This CPU's idle task 
									// Tarea inactiva de la CPU (Cuando no hay ninguna tarea ejecutable) 
								*/
	bool need_resched;			/* Flag activated when a user preemption must take place 
									// Se debe poner a true cuando se debe proceder a replanificar (elegir una nueva tarea para su ejecución)
								*/
	int nr_runnable; 			/* Keeps track of the number of runnable task in this CPU
                     			    -> Note that current is not on the RQ
                     			    // Almacena el numero actual de tareas en la cola
                     			*/
	int next_load_balancing;	/* Timestamp of the next simulation step
                                    where load_balancing will take place
									// Especifica en que momento debe haber un nuevo equilibrado de carga
                                */
	void* rq_cs_data;			/* Pointer enabling a scheduling class to store private data if needed 
									// Permite que el algoritmo de planificación apunte a una estructura
								*/
	pthread_mutex_t lock;       /* Runqueue lock
									// Cerrojo para acceder a los campos de la estructura
								*/
} runqueue_t;
