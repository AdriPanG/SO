...
char use[]="Usage: tar -c|x|e|d -f file_mytar [file1 file2 ...]\n";
...
int main(int argc, char *argv[]) {
  ...
  while((opt = getopt(argc, argv, "cxedf:")) != -1) {
    switch(opt) {
      case 'c':
      ...
      case 'x':
        ...
        break;
      case 'e':
        flag=(flag==NONE)?ENCRYPT:ERROR;
        break;
      case 'd':
        flag=(flag==NONE)?DESENCRYPT:ERROR;
        break;
      case 'f':
        tarName = optarg;
        break;
  ...
  switch(flag) {
    ...
    case EXTRACT:
      ...
      break;
    case ENCRYPT:
      retCode=encryptTar(nExtra, &argv[optind], tarName);
      break;
    case DESENCRYPT:
      if(nExtra!=0){
        fprintf(stderr,"%s",use);
