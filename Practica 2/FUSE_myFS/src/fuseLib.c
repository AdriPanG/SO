...
static int my_mknod(const char *path, mode_t mode, dev_t device) {
	...
	if(mode == S_IFDIR)
		myFileSystem.nodes[idxNodoI]->fileType = 1;
	else{
		myFileSystem.nodes[idxNodoI]->fileType = 0;
	}
...
static int my_unlink(const char *path) {
	...
	if( (idxNodoI = findFileByName(&myFileSystem, (char*)path+1)) != -1) {
		...
		// Marcamos nodo a libre
		node->freeNode = true;
		// Guardamos en disco los cambios del nodo y le liberamos
		updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]);
		free(node);
 ...
 * 
 * @param path file path
 * @param buff buffer where we have data to read
 * @param size quantity of bytes to read
 * @param offset offset over the reading
 * @param f FUSE structure linked to the opened file
 * */
static int my_read (const char *path, char *buff, size_t size, off_t offset, struct fuse_file_info *f) {
	char buffer[BLOCK_SIZE_BYTES];
	int bytes2Read = size, totalRead = 0;
	NodeStruct *nodo = myFileSystem.nodes[f->fh];
	
	if (nodo->fileType != 0)
		return -EISDIR;
	
	
	fprintf(stderr, "--->>>my_readfile: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, f->fh);
	
	//Comprobamos que el tamaño sea menor o igual que el de los bloques
	if(size + offset <=  nodo->numBlocks * BLOCK_SIZE_BYTES) {
		//Leemos los datos
		while (bytes2Read != 0) {
			int i, currentBlock, offBlock;
			
			currentBlock = nodo->blocks[offset / BLOCK_SIZE_BYTES]; //Numero de bloques del archivo
			offBlock = offset % BLOCK_SIZE_BYTES; //Resto de la division anterior
			
			//Ponemos el puntero sobre el bloque del disco virtual para leerlo
			if ((lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) ||
			(read(myFileSystem.fdVirtualDisk, buffer, BLOCK_SIZE_BYTES) == -1)) {
				perror("Failed lseek/read in my_readfile");
				return -EIO;
			}
			...
			//Descontamos lo que hemos leido
...
struct fuse_operations myFS_operations = {
	...
	.read 		= my_read,						// Read data from an open
	.mkdir		= my_mkdir						// Create a new directory
};
